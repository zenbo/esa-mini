...
func newSearchCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Search posts and optionally save to files.
Team can be omitted if ESA_TEAM is set or a default team is saved via 'esa-mini team set'.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) >= 1 {
				...
				if err != nil {
					return cliError("esa-mini search", err.Error(), "Check config permissions.")
				}
				if resolved == "" {
					return cliError("esa-mini search", "team is required", "Specify team as argument, set ESA_TEAM, or run 'esa-mini team set'.")
				}
				team = resolved
			}
